...
uniform float stage;
vec3 osc(vec2 inp, float am, float scale)
{
	float maxAmp = 0.0;
    float amp = 1.0;
    float freq = scale;
	...
	for(int i = 0; i < 8; i++)
	{
		res += psdnoise(inp.xy*freq, vec2(10.0))*amp;
		maxAmp += amp;
		amp *= 2.0;
		freq *= 2.0;
	}
	...
	res = res * (2.0-1.0)/2.0+(2.0+1.0)/2.0;
	return res;
}
...
void main() {
	vec3 c = vec3(vec2(gl_PointCoord.x*8.0,gl_PointCoord.y*8.0), 2.0)*2.0;
	gl_FragColor = vec4(0.95, 0.15, 0.06, 1.0);
}